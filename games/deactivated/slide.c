...
static const char *validate_desc(const game_params *params, const char *desc)
{
    ...
    while (*desc && *desc != ',') {
        ...
        if (*desc == 'f' || *desc == 'F') {
            desc++;
            if (!*desc) {
               ret = "Expected another character after 'f' in game description";
               goto done;
           }
        }
...
#define HIGHLIGHT_WIDTH (1 + TILESIZE/8)
                            ...
                            int x, int y, int button, bool swapped)
{
    ...
    if (button == LEFT_BUTTON) {
        ...
        if (tx < 0 || tx >= w || ty < 0 || ty >= h || !ISBLOCK(state->board[ty*w+tx]))
     ...
     */
         ...
         */
                for (j = i; j >= 0; j = (ISDIST(state->board[j]) ? j - state->board[j] : -1)) {
                    int jx = (j+pos-ui->drag_anchor) % w;
                    int jy = (j+pos-ui->drag_anchor) / w;
                    int j2;

                    if (jx + dx < 0 || jx + dx >= w || jy + dy < 0 || jy + dy >= h)
     ...
     */
        ...
        for (dist = 0; dist <= distlimit; dist++) {
            ...
            for (s = -1; s <= +1; s += 2) {
                ...
                px = tx + dx;
                py = ty + dy;
                if (px >= 0 && px < w && py >= 0 && py < h && ui->reachable[py*w+px]) {
     ...
     */
        ...
        if (ui->drag_anchor != ui->drag_currpos) {
            ...
            return UI_UPDATE;               /* null move; just update the UI */
        }
...
static float *game_colours(frontend *fe, int *ncolours)
{
    ...
    for (i=0;i<3;i++) {
        ...
        ret[COL_TILE                    * 3 + i] = 0.5F;
        ret[COL_DRAGGING                * 3 + i] = 0.75F;
        ret[COL_DRAGGING_HIGHLIGHT      * 3 + i] = 1.0F;
        ret[COL_DRAGGING_LOWLIGHT       * 3 + i] = 0.5F;
        ret[COL_MAIN                    * 3 + i] = 0.25F;
        ret[COL_MAIN_HIGHLIGHT          * 3 + i] = 0.5F;
        ret[COL_MAIN_LOWLIGHT           * 3 + i] = 0.0F;
        ret[COL_MAIN_DRAGGING           * 3 + i] = 0.75F;
        ret[COL_MAIN_DRAGGING_HIGHLIGHT * 3 + i] = 1.0F;
        ret[COL_MAIN_DRAGGING_LOWLIGHT  * 3 + i] = 0.5F;
        ret[COL_TARGET                  * 3 + i] = 0.9F;
        ret[COL_TARGET_HIGHLIGHT        * 3 + i] = 0.75F;
        ret[COL_TARGET_LOWLIGHT         * 3 + i] = 0.5F;
    }
    ...
    */
    ...
    if (val & FG_WALL) {
        ...
        draw_piecepart(dr, ds, tx, ty, (val >> FG_MAINPIECESH) & PIECE_MASK,
                   cl, cc, ch);
    } else if (val & FG_NORMAL) {
        if (val & FG_DRAGGING) {
            cc = COL_DRAGGING;
            ch = COL_DRAGGING_HIGHLIGHT;
            cl = COL_DRAGGING_LOWLIGHT;
        }
        else {
            cc = COL_TILE;
            ch = COL_HIGHLIGHT;
