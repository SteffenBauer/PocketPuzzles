...
enum {
    ...
    COL_TEXT_DARK,
    COL_TEXT_LIGHT,
    COL_TEXT_DARK_UNSOLVED,
    COL_TEXT_LIGHT_UNSOLVED,
    COL_ERROR,
    NCOLOURS
};
...
static bool game_fetch_preset(int i, char **name, game_params **params)
{
    ...
    const int levels[6] = {0, 0, 0, 0, 0, 0};
...
static char *encode_params(const game_params *params, bool full)
{
    char encoded[20] = "";
    if (full) {
        sprintf(encoded, "%dx%dl%d", params->width, params->height, params->level);
    } else {
...
static void draw_cell(drawing *dr, game_drawstate *ds,
    ...
    if (cell & STATE_MARKED) {
        if ((curr && curr->shown) && !(cell & STATE_SOLVED)) {
    ...
    if (cell & STATE_ERROR) {
        draw_rect(dr, startX+ts/12, startY+ts/12, 10*ts/12, 10*ts/12, COL_UNMARKED);
        draw_rect(dr, startX+2*ts/12, startY+2*ts/12, 8*ts/12, 8*ts/12, color);
    }
