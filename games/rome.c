...
static game_state *execute_move(const game_state *oldstate, const char *move)
{
    ...
    if(move[0] == 'M') {
        ...
        state = dup_game(oldstate);
        while(i < w*h) {
            if(!(state->grid[i] & FM_FIXED) && !(state->grid[i] & FM_ARROWMASK))
                state->marks[i] |= FM_ARROWMASK;
            i++;
        }
        return state;
...
static float *game_colours(frontend *fe, int *ncolours)
{
    ...
    for (i=0;i<3;i++) {
        ...
        ret[COL_ARROW_FIXED * 3 + i] = 0.0F;
        ret[COL_ARROW_GUESS * 3 + i] = 0.25F;
        ret[COL_ERRORBG     * 3 + i] = 0.25F;
        ret[COL_GOALBG      * 3 + i] = 0.75F;
        ret[COL_ARROW_ERROR * 3 + i] = 0.75F;
        ret[COL_ARROW_PENCIL* 3 + i] = 0.25F;
        ret[COL_GOAL        * 3 + i] = 0.0F;
    }
...
{
    double thick = size <= 8 ? 2 : 5;
    double sd = size * SIDE_SIZE;
    int color;
    if(ink == -1)
        color = data & FM_FIXED  ? COL_ARROW_FIXED :
                data & FE_DOUBLE ? COL_ARROW_ERROR :
                data & FE_BOUNDS ? COL_ARROW_ERROR :
                                   COL_ARROW_GUESS;
    else
        color = ink;
            ...
            float animtime, float flashtime)
{
    ...
    hchanged = ((ds->hx != ui->hx) || (ds->hy != ui->hy) || (ds->hshow != ui->hshow) || (ds->hpencil != ui->hpencil));
    ...
    for(x = 0; x < w; x++)
    {
        ...
        if (hchanged && (x == ds->hx) && (y == ds->hy)) stale = true;
        if (hchanged && (x == ui->hx) && (y == ui->hy)) stale = true;
        if (c != ds->oldgrid[i1]) stale = true;
        if (p != ds->oldpencil[i1]) stale = true;
        
        if (stale) {
            bool on_hint = (x==ui->hx) && (y==ui->hy);
            cx = (x+0.5) * tilesize;
            cy = (y+0.5) * tilesize;
            cw = tilesize - 1;
            ch = tilesize - 1;
            ...
            if (ui->hpencil && (x==ui->hx) && (y==ui->hy)) {
                int coords[6];
                coords[0] = cx;                     coords[1] = cy;
                coords[2] = coords[0] + tilesize/2; coords[3] = coords[1];
                coords[4] = coords[0];              coords[5] = coords[1] + tilesize/2;
            ...
            if((c & FD_KBMASK) == c) {
                ...
                if(p & FM_LEFT)
                    rome_draw_arrow(dr, (x+0.75)*tilesize, (y+1)*tilesize, tilesize*0.12, FM_LEFT, COL_ARROW_PENCIL);
                if(p & FM_RIGHT)
